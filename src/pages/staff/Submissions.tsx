    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Farmer requests allocated to you by the admin.</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <Card className="mt-6"><CardContent className="p-10 text-center text-muted-foreground">No submissions assigned to you yet.</CardContent></Card>
      ) : (
            ...
            <Card key={s.id}>
              <CardContent className="p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${tone(s.status)}`}>{s.status}</span>
                </div>
                ...
                <div className="mt-4 max-w-xs">
                  <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Update status</p>
