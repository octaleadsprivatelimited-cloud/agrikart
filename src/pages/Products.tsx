...
const CATEGORY_ICONS: Record<Category | "All", typeof Layers> = {
  All: Layers, Pesticides: Bug, Fertilisers: Beaker, Implements: Wrench,
  Organic: Leaf, "Speciality Nutrients": FlaskConical, Veterinary: Stethoscope,
  Seeds: Sparkles, Nano: Atom,
};
        ...
        <div className="container mx-auto px-3 py-3 sm:px-4 sm:py-7">
          ...
          <div className="flex gap-2.5 overflow-x-auto pb-1 sm:gap-5 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            {TILE_ORDER.map(key => {
              ...
              return (
                <button key={key} onClick={() => setCat(key)} className="group flex shrink-0 flex-col items-center gap-1.5 sm:gap-2">
                  <span className={`grid h-12 w-12 place-items-center rounded-full border-2 bg-emerald-100/70 text-emerald-800 transition sm:h-20 sm:w-20 ${isActive ? "border-amber-400 shadow-elegant" : "border-primary/70 group-hover:border-primary"}`}>
                    <Icon className="h-5 w-5 sm:h-9 sm:w-9" strokeWidth={1.8} />
                  </span>
                  <span className={`whitespace-nowrap text-[10px] font-semibold sm:text-sm ${isActive ? "text-primary" : "text-foreground/80"}`}>{t(CATEGORY_I18N[key])}</span>
                </button>
              );
            })}
          </div>
      ...
      <section className="container mx-auto grid gap-5 px-3 py-5 sm:px-4 sm:py-8 lg:grid-cols-[260px_1fr]">
        <aside className="hidden lg:block">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button onClick={() => { setCat("All"); setQ(""); }} className="text-xs font-semibold text-primary hover:underline">{t("shop.clearAll")}</button>
          </div>
          <Card className="overflow-hidden border-emerald-200/60">
            <div className="bg-emerald-100/60 px-4 py-2.5 text-sm font-bold">{t("shop.allCategories")}</div>
            <div className="p-3">
              <div className="relative mb-3">
                <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t("shop.searchPlaceholder")} className="h-9 pr-9 text-sm" />
                ...
                {CATEGORIES.map(c => (
                  <li key={c}>
                    <label className="flex cursor-pointer items-center gap-3 rounded-md px-2 py-2 text-sm hover:bg-muted">
                      <input type="checkbox" checked={cat === c} onChange={() => setCat(cat === c ? "All" : c)} className="h-4 w-4 accent-primary" />
                      <span className="text-xs font-semibold uppercase tracking-wide">{t(CATEGORY_I18N[c])}</span>
                    </label>
                  </li>
                ))}
              </ul>
        ...
        <div>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h1 className="text-lg font-bold sm:text-2xl">
              {activeLabel}{" "}
              <span className="font-medium text-muted-foreground">{t("shop.totalItems", { count: mounted ? filtered.length : 0 })}</span>
            </h1>
            <div className="relative">
              <select value={sort} onChange={(e) => setSort(e.target.value as typeof sort)} className="appearance-none rounded-md border bg-background py-2 pl-3 pr-9 text-xs font-semibold sm:text-sm">
          ...
          {!mounted ? (
            <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 xl:grid-cols-4">
              {Array.from({ length: 8 }).map((_, i) => (<Card key={i} className="aspect-[3/4] animate-pulse bg-muted/60" />))}
            </div>
          ) : filtered.length === 0 ? (
              ...
              {filtered.map(p => {
                ...
                return (
                  <Card key={p.id} className="group relative flex flex-col overflow-hidden rounded-xl border-border/60 bg-white shadow-sm transition hover:shadow-lg">
                    {popular && (
                      <span className="absolute left-2 top-2 z-10 rounded-md bg-sky-100 px-1.5 py-0.5 text-[9px] font-bold text-sky-700 sm:left-3 sm:top-3 sm:px-2 sm:text-[10px]">{t("shop.popular")}</span>
                    )}
                    ...
                    <Link to={`/products/${p.slug}`} className="block">
                      <div className="m-1.5 grid aspect-square place-items-center rounded-lg bg-emerald-50 text-4xl sm:m-3 sm:text-7xl">{p.image}</div>
                    </Link>
                    <div className="flex flex-1 flex-col px-2.5 pb-2.5 sm:px-4 sm:pb-4">
                      <Link to={`/products/${p.slug}`}>
                        <h3 className="line-clamp-2 min-h-[32px] text-[11px] font-bold uppercase tracking-wide text-foreground group-hover:text-primary sm:min-h-[36px] sm:text-sm">{p.name}</h3>
                      </Link>
                      ...
                      <Button asChild size="sm" variant="outline" className="mt-1.5 h-8 w-full gap-1 border-primary/40 px-2 text-[11px] font-semibold text-primary hover:bg-primary hover:text-primary-foreground sm:mt-2 sm:h-9 sm:text-sm">
                        <Link to={`/products/${p.slug}`}>{t("shop.viewDetails")} <ArrowRight className="h-3 w-3 sm:h-3.5 sm:w-3.5" /></Link>
                      </Button>
                    </div>
                  </Card>
                );
