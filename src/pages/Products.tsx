            ...
            {TILE_ORDER.map(key => {
              ...
              return (
                <button key={key} onClick={() => setCat(key)} className="group flex shrink-0 flex-col items-center gap-1.5 sm:gap-2">
                  ...
                  <span className={`whitespace-nowrap text-[10px] font-semibold sm:text-sm ${isActive ? "text-primary" : "text-foreground/80"}`}>{key}</span>
                </button>
              );
            })}
          </div>
      ...
      <section className="container mx-auto grid gap-5 px-3 py-5 sm:px-4 sm:py-8 lg:grid-cols-[260px_1fr]">
        <aside className="hidden lg:block">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button onClick={() => { setCat("All"); setQ(""); }} className="text-xs font-semibold text-primary hover:underline">Clear All</button>
          </div>
        ...
        <div>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h1 className="text-lg font-bold sm:text-2xl">
              {activeLabel}{" "}
              <span className="font-medium text-muted-foreground">Total {mounted ? filtered.length : "—"} Item(s)</span>
            </h1>
              ...
              {filtered.map(p => {
                ...
                return (
                  <Card key={p.id} className="group relative flex flex-col overflow-hidden rounded-xl border-border/60 bg-white shadow-sm transition hover:shadow-lg">
                    {popular && (
                      <span className="absolute left-2 top-2 z-10 rounded-md bg-sky-100 px-1.5 py-0.5 text-[9px] font-bold text-sky-700 sm:left-3 sm:top-3 sm:px-2 sm:text-[10px]">Popular</span>
                    )}
                    ...
                    <div className="flex flex-1 flex-col px-2.5 pb-2.5 sm:px-4 sm:pb-4">
                      ...
                      <Button asChild size="sm" variant="outline" className="mt-1.5 h-8 w-full gap-1 border-primary/40 px-2 text-[11px] font-semibold text-primary hover:bg-primary hover:text-primary-foreground sm:mt-2 sm:h-9 sm:text-sm">
                        <Link to={`/products/${p.slug}`}>View Details <ArrowRight className="h-3 w-3 sm:h-3.5 sm:w-3.5" /></Link>
                      </Button>
                    </div>
                  </Card>
                );
