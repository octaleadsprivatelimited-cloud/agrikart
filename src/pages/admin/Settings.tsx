    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Company profile, payment, shipping & compliance.
        </p>
      </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Account Number">
              <Input
                ...
                onChange={(e) => setS({ ...s, bankAccountNumber: e.target.value })}
              />
            </Field>
            <Field label="Account Type">
              <Input
                value={s.bankAccountType}
                onChange={(e) => setS({ ...s, bankAccountType: e.target.value })}
              />
            </Field>
            <Field label="IFSC Code">
              <Input
                value={s.bankIfsc}
                onChange={(e) => setS({ ...s, bankIfsc: e.target.value })}
              />
            </Field>
          </div>
        </CardContent>
      ...
      <Card>
        <CardContent className="space-y-3 p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Licenses" className="sm:col-span-2">
              <Textarea
                rows={2}
                value={s.license}
                onChange={(e) => setS({ ...s, license: e.target.value })}
              />
            </Field>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-3 p-6">
          ...
          <div className="flex items-center justify-between rounded-md border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Allow COD at checkout</p>
            </div>
          ...
          <div className="flex items-center justify-between rounded-md border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Razorpay (to be integrated)</p>
            </div>
            <Switch
              checked={s.onlineEnabled}
              onCheckedChange={(v) => setS({ ...s, onlineEnabled: v })}
            />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-3 p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Free shipping above (₹)">
              <Input
                type="number"
                value={s.freeShippingAbove}
                onChange={(e) => setS({ ...s, freeShippingAbove: +e.target.value })}
              />
            </Field>
            <Field label="Shipping fee (₹)">
              <Input
                type="number"
                value={s.shippingFee}
                onChange={(e) => setS({ ...s, shippingFee: +e.target.value })}
              />
            </Field>
          </div>
        </CardContent>
      ...
      <Card>
        <CardContent className="space-y-3 p-6">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            . Current roles: Admin, Employee, Farmer.
          </p>
          <ul className="text-sm space-y-1 text-muted-foreground list-disc pl-5">
            ...
            <li>
              <strong>Employee</strong> — customers, requests, payments collection.
            </li>
            <li>
              <strong>Farmer</strong> — customer portal only.
            </li>
          </ul>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="space-y-3 p-6">
          ...
          <p className="text-xs text-muted-foreground">
            Signed in as <strong>{me?.email ?? "—"}</strong>. Use a strong password (6+ characters).
          </p>
