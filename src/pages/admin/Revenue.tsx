...
export default function AdminRevenue() {
  ...
  const submitRefund = () => {
    ...
    toast.success(
      `Refunded ${fmt(refunding.amount)} to ${refunding.farmerName ?? refunding.farmerId}`,
    );
    setRefunding(null);
    setReason("");
  };
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `agrikart-payments-${Date.now()}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            All transactions, refunds, and ledger activity.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat
          ...
          sub="All succeeded txns"
          Icon={IndianRupee}
          tone="bg-[oklch(0.93_0.08_145)] text-[oklch(0.40_0.13_150)]"
        />
        <Stat
          label="Refunds"
          value={fmt(stats.refundsTotal)}
          sub={`${payments.filter((p) => p.status === "Refunded").length} txns`}
          Icon={RotateCcw}
          tone="bg-destructive/10 text-destructive"
        />
        <Stat
          label="Joining / Renewal"
          value={`${fmt(stats.joining)} / ${fmt(stats.renewal)}`}
          Icon={TrendingUp}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <Input
            placeholder="Search txn id, farmer id, name, mobile…"
            className="pl-9"
            value={q}
                ...
                {filtered.map((p) => (
                  <tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/40">
                    ...
                    <td className="px-4 py-3 hidden lg:table-cell text-xs text-muted-foreground">
                      ...
                      {p.email && (
                        <p className="truncate max-w-[180px]" title={p.email}>
                          {p.email}
                        </p>
                      )}
                      {!p.mobile && !p.email && "—"}
                    ...
                    <td
                      className={`px-4 py-3 text-right font-semibold ${p.status === "Refunded" ? "line-through text-muted-foreground" : ""}`}
                    >
                      {fmt(p.amount)}
                    </td>
                    <td className="px-4 py-3">
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Refunding {refunding && fmt(refunding.amount)} to{" "}
              {refunding?.farmerName ?? refunding?.farmerId}. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
