  ...
  const data = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dayOrders = orders.filter(
        (o) => new Date(o.createdAt).toISOString().slice(0, 10) === k && o.status !== "Cancelled",
      );
      days.push({
    ...
    orders
      ...
      .forEach((o) => {
        const k = new Date(o.createdAt).toLocaleDateString("en-IN", {
          month: "short",
          year: "2-digit",
        });
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Daily & monthly sales, top products, exports.
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => exportCsv(data.days, "daily-sales.csv")}>
            <Download className="h-4 w-4" /> Daily CSV
          </Button>
          <Button variant="outline" onClick={() => exportCsv(data.monthly, "monthly-sales.csv")}>
            <Download className="h-4 w-4" /> Monthly CSV
          </Button>
          <Button
            variant="outline"
            onClick={() =>
                  ...
                  data.topProducts.map((p) => (
                    <tr key={p.name}>
                      ...
                      <td className="px-4 py-3">
                        <Badge variant="outline">{p.sold}</Badge>
                      </td>
