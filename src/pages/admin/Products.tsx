...
import {
  useProducts, upsertProduct, deleteProduct, CATEGORIES, fmt, type Product, type Category,
} from "@/lib/shop-store";
...
const empty: Form = {
  name: "", category: "Pesticides", brand: "", manufacturer: "",
  price: 0, stock: 0, reorderLevel: 10,
  batchNumber: "", expiryDate: new Date(Date.now() + 365 * 86400000).toISOString().slice(0, 10),
  license: "", description: "", usage: "", safety: "",
  image: "ðŸ§ª", status: "Active", supplier: "", cropTags: [],
};
...
export default function AdminProducts() {
  ...
  const startEdit = (p: Product) => { setForm({ ...p }); setOpen(true); };
  const save = () => {
                ...
                {filtered.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3">
                        <span className={out ? "text-destructive font-medium" : low ? "text-[oklch(0.55_0.16_75)] font-medium" : ""}>
                          {p.stock}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">{p.expiryDate}</td>
                      <td className="px-4 py-3"><Badge variant={p.status === "Active" ? "default" : "secondary"}>{p.status}</Badge></td>
                      <td className="px-4 py-3 text-right">
                        <Button variant="ghost" size="sm" onClick={() => startEdit(p)}><Pencil className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="sm" onClick={() => remove(p)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                      </td>
                    </tr>
                  );
                })}
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-muted-foreground">No products match.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="License"><Input value={form.license} onChange={e => setForm({ ...form, license: e.target.value })} /></Field>
            <Field label="Supplier"><Input value={form.supplier} onChange={e => setForm({ ...form, supplier: e.target.value })} /></Field>
            <Field label="Image (emoji or URL)"><Input value={form.image} onChange={e => setForm({ ...form, image: e.target.value })} /></Field>
            <Field label="Crop Tags (comma separated)" className="sm:col-span-2">
              <Input value={form.cropTags.join(", ")} onChange={e => setForm({ ...form, cropTags: e.target.value.split(",").map(s => s.trim()).filter(Boolean) })} />
            </Field>
            ...
            <Field label="Usage Instructions" className="sm:col-span-2"><Textarea rows={2} value={form.usage} onChange={e => setForm({ ...form, usage: e.target.value })} /></Field>
            <Field label="Safety Precautions" className="sm:col-span-2"><Textarea rows={2} value={form.safety} onChange={e => setForm({ ...form, safety: e.target.value })} /></Field>
          </div>
          <DialogFooter>
