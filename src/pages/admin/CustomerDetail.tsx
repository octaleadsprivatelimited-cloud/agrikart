...
export default function AdminCustomerDetail() {
  ...
  const approve = () => {
    if (!staff) return;
    if (!hasDocs) {
      toast.error("Cannot approve: farmer has not submitted KYC documents.");
      return;
    }
    try {
      updateCustomerStatus(customer.id, "Approved", staff, remarks.trim() || undefined);
  ...
  const reject = () => {
    if (!staff) return;
    try {
      updateCustomerStatus(customer.id, "Rejected", staff, remarks.trim() || undefined);
              ...
              <p className="mt-1 text-xs">
                {customer.farmerCode ? (
                  ...
                  <span className="inline-block rounded bg-muted px-2 py-0.5 font-medium text-muted-foreground">
                    Farmer ID will be issued after approval
                  </span>
                )}
              </p>
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-4">
            ...
            <Textarea
              ...
              placeholder="e.g. Verified Aadhaar and land documents."
            />
          </div>
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            ...
            {new Date(customer.gps.timestamp).toLocaleString()}
          </p>
          <div className="mt-4">
            <CustomerMapClient
            ...
            {edits.map((e) => (
              <li key={e.id} className="border-l-2 border-primary/40 pl-4">
                <p className="text-sm font-medium">
                  {e.editorName}{" "}
                  <span className="text-xs uppercase text-muted-foreground">({e.editorRole})</span>
                </p>
      ...
      <CardContent className="p-6">
        ...
        {!d ? (
          <p className="mt-3 text-sm text-muted-foreground">
            No documents on file (legacy record). Ask the farmer to re-enroll to upload Aadhaar, PAN
            and land proof.
          </p>
        ) : (
            ...
            {orders.map((o) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {new Date(o.createdAt).toLocaleString()} · {o.paymentMode} · {o.paymentState}
                  </p>
                </div>
