    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {tickets.length} tickets · {tickets.filter((t) => t.status !== "Closed").length} open
        </p>
      </div>
      <Card>
                  ...
                  <tr key={t.id}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Button
                        ...
                        onClick={() => {
                          setView(t);
                          setStatus(t.status === "Open" ? "In Progress" : t.status);
                        }}
                      >
                        Open
                      </Button>
            ...
            <div className="space-y-3 text-sm">
              <div className="rounded-md border bg-muted/30 p-3">
                <p className="text-xs text-muted-foreground">
                  {view.userName} · {view.topic} {view.orderId ? `· ${view.orderId}` : ""}
                </p>
              ...
              <div className="space-y-2">
                {view.replies.map((r, i) => (
                  <div
                    key={i}
                    className={`rounded-md p-3 text-sm ${r.author === view.userName ? "bg-muted" : "bg-primary/5"}`}
                  >
                    <p className="text-xs text-muted-foreground">
