...
export default function AdminDeliveries() {
  ...
  const list = useMemo(
    () => orders.filter((o) => ["Packed", "Shipped", "Delivered"].includes(o.status)),
    [orders],
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Assign delivery staff, track and capture proof.
        </p>
      </div>
                ...
                {list.map((o) => (
                  <tr key={o.id}>
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                      {o.address.village}, {o.address.district}
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs">
                      {o.deliveryPerson ?? "—"}
                      {o.expectedDelivery
                        ? ` · ETA ${new Date(o.expectedDelivery).toLocaleDateString()}`
                        : ""}
                    </td>
                    <td className="px-4 py-3 text-right space-x-1">
                      ...
                      {o.status === "Shipped" && (
                        <Button size="sm" onClick={() => setProofOpen(o)}>
      ...
      <Dialog open={!!open} onOpenChange={(v) => !v && setOpen(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Assign delivery — {open?.id}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={submitAssign}>Assign & ship</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!proofOpen} onOpenChange={(v) => !v && setProofOpen(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delivery proof — {proofOpen?.id}</DialogTitle>
          </DialogHeader>
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="e.g. OTP 4821, signed by farmer"
            />
          </div>
          <DialogFooter>
            ...
            <Button onClick={submitProof}>Confirm delivered</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
