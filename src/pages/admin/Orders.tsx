                ...
                {filtered.map((o) => (
                  <tr key={o.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <span className="text-xs text-muted-foreground">{o.paymentState}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Button variant="ghost" size="sm" onClick={() => setView(o)}>
          ...
          {view && (
            <div className="space-y-4 text-sm">
              ...
              <div className="rounded-md border bg-muted/30 p-3">
                ...
                <p className="mt-1">
                  {view.address.name}, {view.address.line1}, {view.address.village},{" "}
                  {view.address.mandal}, {view.address.district} - {view.address.pincode}
                </p>
              </div>
              <div>
                ...
                <ul className="divide-y rounded-md border">
                  {view.lines.map((l) => (
                    <li key={l.productId} className="flex justify-between p-3">
                      <span>
                        {l.name} Ã— {l.qty}
                      </span>
                      <span className="font-medium">{fmt(l.price * l.qty)}</span>
                    </li>
                  ))}
                </ul>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {NEXT[view.status].map((ns) => (
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      updateOrderStatus(view.id, ns);
                      toast.success(`Order ${ns}`);
                      setView({ ...view, status: ns });
                    }}
                  >
                    {ns}
                  </Button>
                ...
                {view.paymentState === "Pending" && view.paymentMode === "COD" && (
                  <Button
                    ...
                    variant="secondary"
                    onClick={() => {
                      setOrderPayment(view.id, "Paid" as PaymentState);
                      toast.success("Marked Paid");
                      setView({ ...view, paymentState: "Paid" });
                    }}
                  >
                    Mark Paid
                  </Button>
                ...
                <ul className="space-y-1 text-xs">
                  {view.history.map((h, i) => (
                    <li key={i} className="text-muted-foreground">
                      {new Date(h.ts).toLocaleString()} — {h.note}
                    </li>
                  ))}
                </ul>
              </div>
