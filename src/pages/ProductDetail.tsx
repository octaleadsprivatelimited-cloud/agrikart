...
export default function ProductDetail() {
  ...
  const handleAdd = () => {
    if (!user) {
      toast.info("Please login as a farmer to place orders");
      navigate("/login");
      return;
    }
      ...
      <section className="container mx-auto grid gap-8 px-4 py-10 lg:grid-cols-[1fr_1.2fr]">
        <Card>
          <CardContent className="p-0">
            <div className="grid aspect-square place-items-center bg-gradient-to-br from-muted to-background text-[12rem]">
              {p.image}
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-end gap-3">
            ...
            {p.discountPrice && (
              <p className="text-lg text-muted-foreground line-through">{fmt(p.price)}</p>
            )}
          ...
          <div className="grid gap-2 rounded-lg border bg-muted/30 p-4 text-sm sm:grid-cols-2">
            ...
            <Detail
              Icon={Calendar}
              label="Expiry"
              value={new Date(p.expiryDate).toLocaleDateString("en-IN", {
                year: "numeric",
                month: "short",
                day: "numeric",
              })}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="lg" variant="outline" asChild>
              <a href="tel:+919876543210">
                <Phone className="h-4 w-4" /> Enquire by call
              </a>
            </Button>
          </div>
          ...
          <Card className="border-destructive/30 bg-destructive/5">
            <CardContent className="space-y-2 p-4 text-sm">
              ...
              <p className="text-xs text-muted-foreground">
                Pesticides & agrochemicals must be handled by trained adults only. Keep out of reach
