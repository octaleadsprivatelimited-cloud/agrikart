...
export default function Signup() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    if (!/^[0-9]{10}$/.test(form.mobile)) {
      toast.error(t("validation.mobile10"));
      return;
    }
    if (!/^[0-9]{12}$/.test(form.aadhaar)) {
      toast.error(t("validation.aadhaar12"));
      return;
    }
  ...
  const ph = t("register.placeholders", { returnObjects: true }) as {
    survey: string;
    crop: string;
    season: string;
  };
        ...
        <CardContent className="p-5 sm:p-8">
          ...
          {farmerId ? (
            ...
            <form onSubmit={onSubmit} className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label={f.mobile} id="mobile">
                <Input
                  id="mobile"
                  required
                  pattern="[0-9]{10}"
              ...
              <Field label={f.aadhaar} id="aadhaar" className="sm:col-span-2">
                <Input
                  id="aadhaar"
                  required
                  pattern="[0-9]{12}"
                  maxLength={12}
                  value={form.aadhaar}
                  onChange={upd("aadhaar")}
                />
              </Field>
              <Field label={f.village} id="village">
                <Input id="village" required value={form.village} onChange={upd("village")} />
              </Field>
              <Field label={f.mandal} id="mandal">
                <Input id="mandal" required value={form.mandal} onChange={upd("mandal")} />
              </Field>
              <Field label={f.district} id="district">
                <Input id="district" required value={form.district} onChange={upd("district")} />
              </Field>
              <Field label={f.state} id="state">
                <Input id="state" required value={form.state} onChange={upd("state")} />
              </Field>
              <Field label={f.landArea} id="land">
                <Input
                  id="land"
                  required
                  type="number"
                  ...
                  step="0.1"
                  value={form.landArea}
                  onChange={upd("landArea")}
                />
              </Field>
              <Field label={f.surveyNumbers} id="survey">
                <Input
                  id="survey"
                  required
                  value={form.surveyNumbers}
                  onChange={upd("surveyNumbers")}
                  placeholder={ph.survey}
                />
              </Field>
              <Field label={f.mainCrop} id="crop">
                <Input
                  id="crop"
                  required
                  value={form.mainCrop}
                  onChange={upd("mainCrop")}
                  placeholder={ph.crop}
                />
              </Field>
              <Field label={f.season} id="season">
                <Input
                  id="season"
                  required
                  value={form.season}
                  onChange={upd("season")}
                  placeholder={ph.season}
                />
              </Field>
      ...
      <aside className="space-y-4">
        <Card>
          <CardContent className="p-6">
            ...
            <p className="mt-4 rounded-md bg-accent/30 p-3 text-xs text-accent-foreground">
              {t("register.note")}
            </p>
          </CardContent>
        </Card>
